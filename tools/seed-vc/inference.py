...
os.environ['HF_HUB_CACHE'] = '/private/tmp/seed-vc/hf_cache'
import shutil
import warnings
import argparse
import torch
...
def main(args):
    ...
    while processed_frames < cond.size(1):
        ...
        # MPS doesn't support autocast, use a dummy context manager for MPS
        if device.type == 'mps':
            from contextlib import nullcontext
            autocast_context = nullcontext()
        else:
            autocast_context = torch.autocast(device_type=device.type, dtype=torch.float16 if fp16 else torch.float32)
