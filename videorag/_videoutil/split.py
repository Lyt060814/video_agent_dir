...
def saving_video_segments(
    ...
    try:
        print(f"[DEBUG] Starting saving video segments for: {video_name}")
        print(f"[DEBUG] Number of segments to save: {len(segment_index2name)}")
        with VideoFileClip(video_path) as video:
            video_segment_cache_path = os.path.join(working_dir, '_cache', video_name)
        ...
        print(f"[DEBUG] Finished saving all video segments for: {video_name}")
    except Exception as e:
        print(f"[ERROR] Error in saving_video_segments: {str(e)}")
        import traceback
        traceback.print_exc()
